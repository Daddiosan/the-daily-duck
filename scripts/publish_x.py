...
def build_post_text(
    ...
    if len(post_text) > 1000:
        raise ValueError(
            "Generated X post is unexpectedly "
            f"long ({len(post_text)} characters)."
        )
...
def find_existing_post(
    ...
    This closes the important crash window where X accepted
    a previous post but GitHub did not get a chance to commit
    x_posted=True.
    """
    ...
    if (
        ...
        raise RuntimeError(
            "X duplicate preflight could not "
            "resolve user ID: "
            f"{me.text[:1000]}"
        )
...
def main() -> int:
    ...
    #
    # This protects against a previous successful
    # X post whose local GitHub state failed to commit
    # because the workflow was interrupted.
