...
def get_story_title(
    ...
    if isinstance(compact, dict):
        return first_text(
            compact.get("title_en"),
            compact.get("title"),
            compact.get("title_ja"),
...
def build_concept_email(
    ...
    subject = (
        "The Daily Duck — Choose Image Concept — "
        f"{issue_date}"
    )
    ...
    text_lines = [
        ...
        "",
        "Choose ONE image concept from the five options below.",
        "以下の5案から、使用する画像コンセプトを1つ選んでください。",
        "",
    ]
    ...
    # Title ideas are informational here. They are selected later,
    # together with the final real image.
    text_lines += [
        "",
        "=" * 60,
    ...
    text_lines += [
        ...
        "",
        "IMPORTANT:",
        "- Only exact replies 1, 2, 3, 4, or 5 are valid.",
        "- 1〜5 の数字1文字だけが有効です。",
    ...
    html_parts += [
        ...
        "<span style='font-size:26px;font-weight:bold'>1 / 2 / 3 / 4 / 5</span><br><br>",
...
def build_final_email(
    package: dict[str, Any],
) -> tuple[str, str, str, dict[int, Path]]:
    ...
    if not isinstance(previews, list) or len(previews) != 5:
        raise ValueError(
            "Exactly 5 real preview images are required."
        )
...
def send_message(
    ...
    text_body: str,
    html_body: str,
    inline_images: dict[int, Path] | None = None,
) -> int:
    ...
    if inline_images:
        ...
        for n, path in inline_images.items():
        ...
        # Attach originals as well for reliable mobile review.
        for n, path in inline_images.items():
            maintype, subtype = mime_for(
                path
            )
...
def main() -> int:
    ...
    if state == "CONCEPTS_READY":
        ...
        package["concept_email_subject"] = subject
        package["email_subject"] = subject
        package["state"] = "WAITING_CONCEPT_SELECTION"
        ...
        print(
            f"Concept selection email sent to {count} recipient(s)."
        )
        print("Exactly 5 concepts presented.")
        print("Valid replies: 1 / 2 / 3 / 4 / 5")
        print("STATE: WAITING_CONCEPT_SELECTION")
        return 0
    ...
    if state == "DESIGN_PREVIEWS_READY":
        ...
        print(
            f"Final image + title email sent to {count} recipient(s)."
        )
        print("Exactly 5 real images + 3 titles presented.")
        print("Valid selection: image 1-5 + title 1-3")
        print("Regeneration command: NEXT 5")
        print("STATE: WAITING_FINAL_SELECTION")
        return 0

    raise RuntimeError(
        "send_design_approval_email.py cannot send mail from state "
        f"{state!r}. Expected CONCEPTS_READY or DESIGN_PREVIEWS_READY."
    )
