...
def render_card(ready: dict[str, Any]) -> Path:
    ...
    if logo_path.exists():
        ...
        card.paste(logo, (46, 30), logo)
        brand_x = 160
    else:
        brand_x = 56

    draw.text((brand_x, 51), "THE DAILY DUCK", font=font(54, True), fill=NAVY)
    tagline = "ONE DAY. ONE STORY. ONE DUCK."
    tb = draw.textbbox((0,0), tagline, font=font(24, True))
    draw.text((W - 55 - (tb[2]-tb[0]), 64), tagline, font=font(24, True), fill=NAVY)
    draw.line((45, 138, W-45, 138), fill=NAVY, width=3)
    ...
    # Footer
    footer_y = H - 72
    draw.rounded_rectangle((42, footer_y, W-42, H-22), radius=18, fill=NAVY)
    draw.text((76, footer_y+14), "🌐  thedailyduck.ai", font=font(26, True), fill=WHITE)
    draw.text((410, footer_y+14), "|   #TheDailyDuck", font=font(26, True), fill=WHITE)
