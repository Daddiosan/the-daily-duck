(() => {
  ...
  function applyLanguage(nextLang) {
    ...
    toggle.setAttribute('aria-label', lang === 'ja' ? 'Switch to English' : '日本語に切り替える');
    localStorage.setItem('dailyDuckLang', lang);
  }

  toggle.addEventListener('click', () => applyLanguage(lang === 'ja' ? 'en' : 'ja'));
  applyLanguage(lang);
  ...
  shareBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (error?.name !== 'AbortError') {
        shareStatus.textContent = lang === 'ja' ? 'シェアできませんでした' : 'Could not share';
      }
    }
  });
})();
